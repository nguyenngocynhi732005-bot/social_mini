...
class ProfileController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $user = Auth::user();
        $profileId = $requestedId ?: ($user ? $user->getKey() : null);
    ...
    private function renderProfilePage(Request $request, ?int $profileId = null)
    {
        ...
        $targetUserId = (int) $targetUser->getKey();
    ...
    private function resolveEditableUser(Request $request): ?User
    {
        ...
        if ($rawProfileId !== null && $rawProfileId !== '' && is_numeric($rawProfileId)) {
            ...
            if ($requested && $requested->getKey() !== null) {
                return $requested;
            }
        }
    ...
    public function updateAccount(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'email' => ['required', 'email', 'max:255', Rule::unique('users', $emailColumn)->ignore($user->getKey(), $keyName)],
        ...
        DB::table('users')->where($keyName, $user->getKey())->update($updates);
    ...
    public function activityLog(Request $request)
    {
        ...
        if (!$targetUser || $targetUser->getKey() === null) {
